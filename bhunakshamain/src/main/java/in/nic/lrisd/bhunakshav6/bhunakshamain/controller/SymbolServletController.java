...
import in.nic.lrisd.bhunakshav6.bhunakshacommon.service.SymbolServlet;
...
public class SymbolServletController {

    @Autowired
    private SymbolServlet symbolServlet;

    @GetMapping("/{code}")
    public ResponseEntity<byte[]> getSymbolServlet(@PathVariable String code, HttpServletRequest request,
                                                   HttpServletResponse response) {
        SymbolMaster symbolMaster = symbolServlet.getSymbolImage(code);
